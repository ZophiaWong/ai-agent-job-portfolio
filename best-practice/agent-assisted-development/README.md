...
历史上的公共术语 [vibe coding](https://x.com/karpathy/status/1886192184808149383) 描述一种以自然语言驱动、对实现细节介入较少的编程体验。本文不把它当作工程交付方法。这里讨论的是受控的 Agent 辅助开发：人保留目标、边界和验收责任，Agent 在明确范围内帮助查找、计划、修改和检查。
...
- 已实现：确定性验证能阻止最终回答，并触发一次恢复尝试，见 [verification tests](https://github.com/ZophiaWong/forge-harness/blob/9c1b1dbb0566e9053457db50e64cd374848de856/test/runtime/verification.test.ts)。
...
- 已计划：外部工具以及 MCP 或插件路由是后续边界，不属于该已实现 checkpoint。
...
把下面内容当作测量候选指标，而不是已经得到的结果：任务完成时间、返工率、被 review 拒绝的 patch 比例、回归数量、无关 diff 数量。开始记录前，先定义任务类型、样本期、基线和失败如何计数；原始记录、命令输出和 diff 要保留到能被复查的位置。

如果以后完成了自己的实践，可以如实这样讲：说明做了什么、给出可检查的提交或测试证据、报告记录方法和局限。没有这些证据时，只能说“我按这个流程练习过”或“我计划这样测量”，不能虚构个人项目经历。
