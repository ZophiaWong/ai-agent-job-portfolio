...
以下进程行为以 **Node 15+，且当前 Node 26.x 默认 `--unhandled-rejections=throw`** 为例：仅在没有
`unhandledRejection` listener 处理该事件时，未处理 rejection 才会按未捕获异常处理。安装
`process.on("unhandledRejection", listener)` 会改变这个 fallback（回退）行为；listener 只用于遥测、
告警和受控 shutdown，不是让业务从未知状态“恢复”的机制。若用 CLI 改成 `warn`、`none` 或 `strict`，进程
结果也会不同。无论版本或 flag，都应在路由、任务入口或调用方 `await`/`.catch()`。
...
按[统一练习协议](../../practice-protocol.md)用 `node --unhandled-rejections=throw promise-evidence.mjs` 运行一个
已捕获和一个未捕获 rejection，并记录 exit code。若无 Node，手动检查每个 Promise 是否被 `await`、`return`
或 `.catch()` 接到业务边界。可检查结果：两条 Promise 链的处理归属和对应的进程配置说明。
